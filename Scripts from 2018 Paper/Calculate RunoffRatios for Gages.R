...
info <- read.csv("USGS_gage_characteristics.csv")
...
#in information file
	#DRAIN_SQKM is area
	#ID is number
	Drain <- info$DRAIN_SQKM[info$ID == as.numeric(IDs$ID[y])]
	Drain <- Drain * 1000000000000
...
for (x in 1:20){
	...
	#parameterCd <- "72194" #precip cumulative (mm)
...
if(length(discharge[,4])>335){
	...
	clim <- info$climate_div_name[info$ID == as.numeric(IDs$ID[y])]
	cdiv <- which(colnames(P)==clim)
	precip <- P[x,cdiv] 
	
	
	discharge[,4] <- discharge[,4]*28316846.59 #convert to mm3/s	

	RO <- (discharge[,4]/Drain) * 86400 #runoff in mm/d
	
	IDs[y,x+1] <- sum(RO) #mm/y
...
write.csv(IDs,"RRs_lowdev_96-16.csv")
