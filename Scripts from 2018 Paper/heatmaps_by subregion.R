...
all <- read.csv("BFI_lowdev_96-16_2AUG.csv")
...
allPD <- allPD[,-24:-22]
#allPD <- allPD[,-29:-24]
allPD <- allPD[order(allPD[,22]),]

allPD_m <- melt(allPD)

#preserve order for plotting by de-alphabetizing the factors
allPD_m$site_name <- as.character(allPD_m$site_name)
allPD_m$site_name <- factor(allPD_m$site_name, levels = unique(allPD_m$site_name))

allMTN <- allMTN[,3:27]
allMTN <- allMTN[,-24:-22]
allMTN <- allMTN[order(allMTN[,22]),]


allMTN_m <- melt(allMTN)

#preserve order for plotting by de-alphabetizing the factors
allMTN_m$site_name <- as.character(allMTN_m$site_name)
allMTN_m$site_name <- factor(allMTN_m$site_name, levels = unique(allMTN_m$site_name))
#########
...
#add barplot of precip to the top of heatmap
barplotPD <- ggplot(data = precip, aes(x = WY, y = MTNS)) +
...
grid.arrange(barplotPD,heatMTN, nrow = 2, ncol = 1, widths = 100, heights = c(10,80))
